...
spl.Nat=function(A,xs,showMatrix=FALSE,showPoly=FALSE,showGraph=FALSE){
    x=A[,1]
    fx=A[,2]
    n=length(x)-1
    a=fx
...
#******************************************************* 
     if(showGraph==TRUE){
       ...
       yGraph=c()
       for(i in 1:n){
          xGraph.aux=c(seq(x[i],x[i+1],abs(x[i+1]-x[i])/20))
          yGraph.aux=a[i]+b[i]*(xGraph.aux-x[i])+c[i]*(xGraph.aux-x[i])^2+d[i]*(xGraph.aux-x[i])^3
          xGraph=c(xGraph,xGraph.aux)
          yGraph=c(yGraph,yGraph.aux)
       }
      
      plot(xGraph,yGraph, col = "black", lwd = 2,type = "l",main = "Spline Natural",xlab = "X",ylab = "SplNat(x)")
